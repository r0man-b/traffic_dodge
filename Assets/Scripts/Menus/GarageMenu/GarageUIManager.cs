...
public class GarageUIManager : MonoBehaviour
{
    ...
    // When true, user is choosing a car to apply an awarded part to.
    // Browsing should skip unowned cars across all types, and CarDisplay will
    ...
    public TextMeshProUGUI paintPurchaseInfoText;
    ...
    public void ChangeCar(int change)
    {
        ...
        // Make sure bucket/state matches the final type/index before fetching the car
        typeBucket = GetCarTypeBucket(currentCarType);
        numOfThisCarTypeOwned = SaveManager.Instance.SaveData.Cars.Count(kv => kv.Key.CarType == currentCarType);
        maxVariations = typeBucket.items.Count;
        ...
        isOwned = saveData.Cars.ContainsKey((currentCarType, currentCarIndex));
        if (isOwned)
        {
        ...
        if (carDisplay != null)
        {
            ...
            if (currentCar == null)
            {
                Debug.LogError($"DisplayCar returned null. Type='{currentCarType}', Index={currentCarIndex}. " +
                               "Check that carPrefabs[] covers all buckets and that BuildCarTypeNameIndex maps all variant names.");
                return;
            }
        }
    ...
    public void SetColor(int paintPrice)
    {
        ...
        if (currentPaintType == 4) // metal bucket
        {
            ...
            if (IsOwnedMetal(saved, presetIndex))
            {
                ...
                paintPopUps.SetActive(false);
                paintPurchaseInfoText.text = "";
                paintPurchaseInfoText.gameObject.SetActive(false);
        ...
        if (currentPaintType == 4)
        {
            paintPurchaseInfoText.text += "Any metal paint purchase persists and can be selected again later at no cost.\n\n";
            paintPurchaseInfoText.gameObject.SetActive(true);
        }

        // Display warning of unique paint removal if applicable
        if (warnUnique)
        {
            paintPurchaseInfoText.text += "Warning: purchasing this paint will overwrite the unique color you have installed.";
            paintPurchaseInfoText.gameObject.SetActive(true);
        }
