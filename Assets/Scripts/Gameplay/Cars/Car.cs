...
public class Car : ScriptableObject
{
    ...
    public Car CreateRuntimeInstance(Transform currentCarModel)
    {
        // Car assets are ScriptableObjects, so their Material fields point at project .mat files.
        // Any color/livery/keyword edits made directly on the asset instance will dirty those files
        // in Unity Version Control after entering Play Mode. Use a cloned Car plus cloned materials
        // for every displayed/spawned car so customization stays scene/runtime-only.
        Car runtimeCar = Instantiate(this);
        runtimeCar.name = name + " (Runtime)";
        runtimeCar.CloneAndBindRuntimeMaterials(currentCarModel);
        return runtimeCar;
    }

    private void CloneAndBindRuntimeMaterials(Transform currentCarModel)
    {
        // One source material should map to one runtime material for this car instance.
        // That keeps the primary/secondary/rim/light slots shared consistently across body parts,
        // while still protecting the imported material assets from Play Mode changes.
        Dictionary<Material, Material> runtimeMaterials = new Dictionary<Material, Material>();

        primColor = GetRuntimeMaterial(primColor, runtimeMaterials);
        secondColor = GetRuntimeMaterial(secondColor, runtimeMaterials);
        rimColor = GetRuntimeMaterial(rimColor, runtimeMaterials);
        primLight = GetRuntimeMaterial(primLight, runtimeMaterials);
        secondLight = GetRuntimeMaterial(secondLight, runtimeMaterials);
        tailLight = GetRuntimeMaterial(tailLight, runtimeMaterials);

        if (currentCarModel == null) return;

        CarPart[] carParts = currentCarModel.GetComponentsInChildren<CarPart>(true);
        foreach (CarPart carPart in carParts)
        {
            // Pre-placed menu cars can already have runtime paint materials from a previous display
            // pass. Register those existing materials as aliases before touching renderers so reused
            // body submeshes still bind to this runtime Car's canonical paint slots.
            RegisterRuntimeMaterialAlias(carPart.primaryColor, primColor, runtimeMaterials);
            RegisterRuntimeMaterialAlias(carPart.secondaryColor, secondColor, runtimeMaterials);
            RegisterRuntimeMaterialAlias(carPart.rimColor, rimColor, runtimeMaterials);
            RegisterRuntimeMaterialAlias(carPart.primaryLight, primLight, runtimeMaterials);
            RegisterRuntimeMaterialAlias(carPart.secondaryLight, secondLight, runtimeMaterials);
            RegisterRuntimeMaterialAlias(carPart.tailLight, tailLight, runtimeMaterials);
        }

        foreach (Renderer renderer in currentCarModel.GetComponentsInChildren<Renderer>(true))
        {
            // Assign through sharedMaterials intentionally: these are scene-instance renderers,
            // and using Renderer.material here would create extra hidden material copies per renderer.
            // The replacement array points every submesh at a runtime material clone, including
            // chrome/gold/glass materials that are not one of the six customizable paint slots.
            ...
            for (int i = 0; i < materials.Length; i++)
            {
                if (materials[i] != null)
                {
                    materials[i] = GetRuntimeMaterial(materials[i], runtimeMaterials);
                    changed = true;
                }
            }
        ...
        foreach (CarPart carPart in carParts)
        {
            carPart.primaryColor = primColor;
            carPart.secondaryColor = secondColor;
            carPart.rimColor = rimColor;
            carPart.primaryLight = primLight;
            carPart.secondaryLight = secondLight;
            carPart.tailLight = tailLight;
        }

        foreach (MaxOutTheEmmission emissionMaxer in currentCarModel.GetComponentsInChildren<MaxOutTheEmmission>(true))
        {
            // Menu cars are reused instead of instantiated fresh. When this method clones
            // renderer materials for a returning car, MaxOutTheEmmission.Start() will not
            // run again, so its cached material can point at the previous display cycle.
            // Refresh it after the renderer swap so animated light offsets keep driving
            // the material currently assigned to the mesh.
            emissionMaxer.RefreshAfterRuntimeMaterialSwap();
        }
    }

    private static Material GetRuntimeMaterial(Material source, Dictionary<Material, Material> runtimeMaterials)
    {
        if (source == null) return null;

        if (!runtimeMaterials.TryGetValue(source, out Material runtimeMaterial))
        {
            // DontSave prevents these Play Mode-only clones from being serialized into the scene,
            // prefab, or material assets when Unity refreshes or version control scans for changes.
    ...
    private static void RegisterRuntimeMaterialAlias(Material source, Material runtimeMaterial, Dictionary<Material, Material> runtimeMaterials)
    {
        if (source == null || runtimeMaterial == null || source == runtimeMaterial) return;
        runtimeMaterials[source] = runtimeMaterial;
    }
